        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div className="bg-white bg-opacity-10 backdrop-blur p-6 rounded-2xl shadow-xl border border-white/20">
            {testimonials.length > 0 ? (
              ...
              <p className="text-center text-white/70">No testimonials available.</p>
            )}
          </div>
          ...
          <div className="text-center md:text-left">
            ...
            <div className="flex justify-center md:justify-start gap-4">
              <button onClick={() => swiperRef.current?.slidePrev()} className="w-10 h-10 bg-white/20 rounded-full">
                ←
              </button>
              <button onClick={() => swiperRef.current?.slideNext()} className="w-10 h-10 bg-white/20 rounded-full">
                →
              </button>
            </div>
          </div>
