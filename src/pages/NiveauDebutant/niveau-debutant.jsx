  ...
  return (
    ...
    >
      <div className="bg-black/60 min-h-screen p-6">
        ...
        {/* Semester 1 avec navigation vers fiqh */}
        <SectionSemester
          title={t("semester1")}
          subjects={translatedSubjects}
          onClick={(subjectKey) => {
            if (subjectKey === "fiqh") {
              navigate(`/${lang}/niveau-debutant/semester1/fiqh`);
            }
          }}
        />

        {/* Semester 2 sans navigation pour l’instant */}
        <SectionSemester
          title={t("semester2")}
          subjects={translatedSubjects}
        />
...
function SectionSemester({ title, subjects, onClick }) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
        {subjects.map((subject, idx) => (
          <button
            ...
            onClick={() => {
              const subjectKey = ["fiqh", "aqida", "tajwid", "nahw", "sirah", "hadith", "akhlaq"][idx];
              onClick?.(subjectKey);
            }}
          >
            {subject}
