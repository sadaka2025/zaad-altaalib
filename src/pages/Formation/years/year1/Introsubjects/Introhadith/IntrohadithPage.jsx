...
export default function IntroHadithPage() {
  ...
  const semesterData = data?.year1?.hadith?.[semesterKey] || {};
  ...
  // pour S1 / S2
  const semesterStats = hadithStats?.year1?.hadith?.[semesterKey] || {};

  // pour les stats annuelles
  const totalYearStats = hadithStats?.year1?.hadith?.yearStats || {};
  ...
  const goToLesson = (lessonId) => {
    navigate(
      `/ar/annee/1/matiere/hadith?semestre=${selectedSemester === 'semester1' ? 1 : 2}&lesson=${lessonId}`
    );
    ...
    <div className="font-[Arial] max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        <button
          onClick={() =>
            navigate(
              `/ar/annee/1/matiere/hadith?semestre=${selectedSemester === 'semester1' ? 1 : 2}`
            )
          }
