...
import data from '../../../../../../datatext/years/year1/datatxtnahw.json';
import courseDataJSON from '../../../../../../dataIntro/years/year1/dataLesson/lessonListnahw.json';
import nahwStats from '../../../../../../datastat/years/year1/nahw_stats.json';
...
export default function IntroNahwPage() {
  ...
  // ✅ accès sécurisé — on garde niveau intermédiaire "nahw" dans les JSON
  const semesterKey = selectedSemester === 'semester1' ? 's1' : 's2';
  const semesterData = data?.year1?.nahw?.[semesterKey] || {};
  ...
  useEffect(() => {
    ...
    // NOTE: courseDataJSON has intermediate "nahw" according to your request
    const semesterCourseData =
      courseDataJSON?.nahw?.semesters?.[semesterIndex] || {};
    setLessons(
  ...
  // pour S1 / S2 — structure avec niveau "nahw"
  const semesterStats = nahwStats?.year1?.nahw?.[semesterKey] || {};
  const totalYearStats = nahwStats?.year1?.nahw?.yearStats || {};
  ...
  const goToLesson = (lessonId) => {
    if (!lessonId) return;
    navigate(
      `/ar/annee/1/matiere/nahw?semestre=${selectedSemester === 'semester1' ? 1 : 2}&lesson=${lessonId}`
    );
  ...
  const typewriterLines = useMemo(() => {
    ...
    const header = isS1
      ? 'في هذا السداسي الأوّل، نؤسس لعلم النحو وميزان اللغة العربية.'
      : 'في هذا السداسي الثاني، نغوص في التحليل النحوي والنماذج التطبيقية.';
    ...
    const tail = isS1
      ? 'الهدف تبسيط القواعد وتمكين الطالب من قراءة النصوص وفهم إعرابها.'
      : 'نركّز على التمرين العملي وتحليل الجمل والنصوص مع تطبيقات تطبيقية.';

    return [header, ...body, tail];
  }, [sections, semesterData, selectedSemester]);

  if (!data?.year1?.nahw || !nahwStats?.year1?.nahw) {
    return (
      <div className="p-6 text-center text-red-600">
        ⚠️ Les données de nahw ne sont pas disponibles.
      </div>
    );
  }
  ...
  return (
    <div className="font-[Arial] max-w-6xl mx-auto p-4 space-y-6">
      ...
      <section className="grid md:grid-cols-5 gap-3 text-center mt-8">
        ...
        <button
          onClick={() => navigate('/ar/profavis/nahw')}
          className="px-4 py-2 rounded-lg font-bold shadow-md"
          style={{
            backgroundColor: '#fef3c7',
            color: '#3b2f1b',
      ...
      <div className="flex gap-4 justify-center my-6">
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/1/nahw?semestre=1`);
          }}
        >
          📘 السداسي الأول
        </button>
        ...
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/1/nahw?semestre=2`);
          }}
        >
          📗 السداسي الثاني
        </button>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        <button
          onClick={() =>
            navigate(
              `/ar/annee/1/matiere/nahw?semestre=${selectedSemester === 'semester1' ? 1 : 2}`
            )
          }
        ...
        >
          ...
          <div className="bg-neutral-900/95 rounded-2xl p-4 md:p-6 shadow-[0_10px_40px_rgba(0,0,0,0.35)] ring-1 ring-white/10">
            <h2 className="text-center text-white text-2xl md:text-3xl font-bold mb-4">
              {selectedSemester === 'semester1'
                ? 'هذا السداسي الأول يركّز على 4 أجزاء رئيسية في النحو'
                : 'هذا السداسي الثاني يركّز على 4 أجزاء رئيسية في النحو'}
            </h2>
