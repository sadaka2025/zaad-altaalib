...
import data from '../../../../../../datatext/years/year2/datatxthadith.json';
import courseDataJSON from '../../../../../../dataIntro/years/year2/dataLesson/lessonListhadith.json';
import hadithStats from '../../../../../../datastat/years/year2/hadith_stats.json';
...
export default function IntroHadithPage() {
  ...
  const semesterData = data?.year2?.hadith?.[semesterKey] || {};
  ...
  // pour S1 / S2
  const semesterStats = hadithStats?.year2?.hadith?.[semesterKey] || {};

  // pour les stats annuelles
  const totalYearStats = hadithStats?.year2?.hadith?.yearStats || {};
  ...
  const goToLesson = (lessonId) => {
    navigate(
      `/ar/annee/2/matiere/hadith?semestre=${selectedSemester === 'semester1' ? 1 : 2}&lesson=${lessonId}`
    );
  ...
  const typewriterLines = useMemo(() => {
    ...
    const header = isS1
      ? 'في هذا السداسي الأوّل، نتعرّف على علوم الحديث وأصول الرواية.'
      : 'في هذا السداسي الثاني، ندرس مصطلح الحديث وتقييم الراوي وربط النصّ بالممارسة.';
    ...
    const tail = isS1
      ? 'الهدف فهم الأسانيد والمتون وتمكين الطالب من قراءة السند والنقد.'
      : 'نركّز على التمييز بين الصحيح والضعيف وكيفية تطبيق الأحاديث في الحياة.';

    return [header, ...body, tail];
  }, [sections, semesterData, selectedSemester]);

  if (!data?.year2?.hadith) {
    return (
      <div className="p-6 text-center text-red-600">
        ⚠️ Les données d’hadith ne sont pas disponibles.
      </div>
    );
  }
  ...
  return (
    <div className="font-[Arial] max-w-6xl mx-auto p-4 space-y-6">
      ...
      <section className="grid md:grid-cols-5 gap-3 text-center mt-8">
        ...
        <button
          onClick={() => navigate('/ar/profavis/hadith')}
          className="px-4 py-2 rounded-lg font-bold shadow-md"
          style={{
            backgroundColor: '#fef3c7',
            color: '#3b2f1b',
      ...
      <div className="flex gap-4 justify-center my-6">
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/hadith?semestre=1`);
          }}
        >
          📘 السداسي الأول
        </button>
        ...
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/hadith?semestre=2`);
          }}
        >
          📗 السداسي الثاني
        </button>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        <button
          onClick={() =>
            navigate(
              `/ar/annee/2/matiere/hadith?semestre=${selectedSemester === 'semester1' ? 1 : 2}`
            )
          }
        ...
        >
          ...
          <div className="bg-neutral-900/95 rounded-2xl p-4 md:p-6 shadow-[0_10px_40px_rgba(0,0,0,0.35)] ring-1 ring-white/10">
            <h2 className="text-center text-white text-2xl md:text-3xl font-bold mb-4">
              {selectedSemester === 'semester1'
                ? 'هذا السداسي الأول يركّز على 4 أجزاء رئيسية في الحديث'
                : 'هذا السداسي الثاني يركّز على 4 أجزاء رئيسية في الحديث'}
            </h2>
