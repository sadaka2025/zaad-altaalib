...
import data from '../../../../../../datatext/years/year2/datatxtakhlaq.json';
import courseDataJSON from '../../../../../../dataIntro/years/year2/dataLesson/lessonListakhlaq.json';
import akhlaqStats from '../../../../../../datastat/years/year2/akhlaq_stats.json';
...
export default function IntroAkhlaqPage() {
  ...
  const semesterData = data?.year2?.akhlaq?.[semesterKey] || {};
  ...
  // pour S1 / S2
  const semesterStats = akhlaqStats?.year2?.akhlaq?.[semesterKey] || {};

  // pour les stats annuelles
  const totalYearStats = akhlaqStats?.year2?.akhlaq?.yearStats || {};
  ...
  const goToLesson = (lessonId) => {
    navigate(
      `/ar/annee/2/matiere/akhlaq?semestre=${selectedSemester === 'semester1' ? 1 : 2}&lesson=${lessonId}`
    );
  ...
  const typewriterLines = useMemo(() => {
    ...
    const header = isS1
      ? 'في هذا السداسي الأوّل، نمضي خطوة بخطوة لترسيخ أخلاق المسلم ومقاصدها العملية.'
      : 'في هذا السداسي الثاني، نواصل البناء بالأبواب المتقدّمة وربط الأخلاق بالتزكية والسلوك.';
    ...
    const tail = isS1
      ? 'هدفنا إصلاح القلب والسلوك بمنهجٍ متدرّج، مع تطبيقات حياتية وتمارين عملية.'
      : 'نركّز هنا على التثبيت والمراجعة، مع تقييمات مرحلية وتقويم نهائي في الأخلاق.';

    return [header, ...body, tail];
  }, [sections, semesterData, selectedSemester]);

  if (!data?.year2?.akhlaq) {
    return (
      <div className="p-6 text-center text-red-600">
        ⚠️ Les données d’akhlaq ne sont pas disponibles.
      </div>
    );
  }
    ...
    <div className="font-[Arial] max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="flex gap-4 justify-center my-6">
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/akhlaq?semestre=1`);
          }}
        >
          📘 السداسي الأول
        </button>
        ...
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/akhlaq?semestre=2`);
          }}
        >
          📗 السداسي الثاني
        </button>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        <button
          onClick={() =>
            navigate(
              `/ar/annee/2/matiere/akhlaq?semestre=${selectedSemester === 'semester1' ? 1 : 2}`
            )
          }
        ...
        >
          ...
          <div className="bg-neutral-900/95 rounded-2xl p-4 md:p-6 shadow-[0_10px_40px_rgba(0,0,0,0.35)] ring-1 ring-white/10">
            <h2 className="text-center text-white text-2xl md:text-3xl font-bold mb-4">
              {selectedSemester === 'semester1'
                ? 'هذا السداسي الأول يركّز على 4 أجزاء رئيسية في الأخلاق'
                : 'هذا السداسي الثاني يركّز على 4 أجزاء رئيسية في الأخلاق'}
            </h2>
