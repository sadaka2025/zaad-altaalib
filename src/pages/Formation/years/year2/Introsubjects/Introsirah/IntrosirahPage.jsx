...
import data from '../../../../../../datatext/years/year2/datatxtsirah.json';
import courseDataJSON from '../../../../../../dataIntro/years/year2/dataLesson/lessonListsirah.json';
import sirahStats from '../../../../../../datastat/years/year2/sirah_stats.json';
...
export default function IntroSirahPage() {
  ...
  const semesterData = data?.year2?.sirah?.[semesterKey] || {};
  ...
  const semesterStats = sirahStats?.year2?.sirah?.[semesterKey] || {};
  const totalYearStats = sirahStats?.year2?.sirah?.yearStats || {};
  ...
  const goToLesson = (lessonId) => {
    if (!lessonId) return;
    navigate(
      `/ar/annee/2/matiere/sirah?semestre=${selectedSemester === 'semester1' ? 1 : 2}&lesson=${lessonId}`
    );
  ...
  if (!data?.year2?.sirah || !sirahStats?.year2?.sirah) {
    return (
    ...
    <div className="font-[Arial] max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="flex gap-4 justify-center my-6">
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/sirah?semestre=1`);
          }}
        >
          📘 السداسي الأول
        </button>
        ...
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/sirah?semestre=2`);
          }}
        >
          📗 السداسي الثاني
        </button>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        <button
          onClick={() =>
            navigate(
              `/ar/annee/2/matiere/sirah?semestre=${selectedSemester === 'semester1' ? 1 : 2}`
            )
          }
