...
import data from '../../../../../../datatext/years/year2/datatxtaqida.json';
import courseDataJSON from '../../../../../../dataIntro/years/year2/dataLesson/lessonListaqida.json';
import aqidaStats from '../../../../../../datastat/years/year2/aqida_stats.json';
...
export default function IntroAqidaPage() {
  ...
  const semesterData = data?.year2?.aqida?.[semesterKey] || {};
  ...
  // pour S1 / S2
  const semesterStats = aqidaStats?.year2?.aqida?.[semesterKey] || {};

  // pour les stats annuelles
  const totalYearStats = aqidaStats?.year2?.aqida?.yearStats || {};
  ...
  const goToLesson = (lessonId) => {
    navigate(
      `/ar/annee/2/matiere/aqida?semestre=${selectedSemester === 'semester1' ? 1 : 2}&lesson=${lessonId}`
    );
    ...
    <div className="font-[Arial] max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="flex gap-4 justify-center my-6">
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/aqida?semestre=1`);
          }}
        >
          📘 السداسي الأول
        </button>
        ...
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/aqida?semestre=2`);
          }}
        >
          📗 السداسي الثاني
        </button>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        <button
          onClick={() =>
            navigate(
              `/ar/annee/2/matiere/aqida?semestre=${selectedSemester === 'semester1' ? 1 : 2}`
            )
          }
