...
import data from '../../../../../../datatext/years/year2/datatxttajwid.json';
import courseDataJSON from '../../../../../../dataIntro/years/year2/dataLesson/lessonListtajwid.json';
import tajwidStats from '../../../../../../datastat/years/year2/tajwid_stats.json';
...
export default function IntrotajwidPage() {
  ...
  const semesterData = data?.year2?.tajwid?.[semesterKey] || {};
  ...
  const semesterStats = tajwidStats?.year2?.tajwid?.[semesterKey] || {};
  const totalYearStats = tajwidStats?.year2?.tajwid?.yearStats || {};
  ...
  const goToLesson = (lessonId) => {
    if (!lessonId) return;
    navigate(
      `/ar/annee/2/matiere/tajwid?semestre=${selectedSemester === 'semester1' ? 1 : 2}&lesson=${lessonId}`
    );
  ...
  if (!data?.year2?.tajwid || !tajwidStats?.year2?.tajwid) {
    return (
    ...
    <div className="font-[Arial] max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="flex gap-4 justify-center my-6">
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/tajwid?semestre=1`);
          }}
        >
          📘 السداسي الأول
        </button>
        ...
        <button
          ...
          onClick={() => {
            launchFlowers();
            navigate(`/${lang}/intro/2/tajwid?semestre=2`);
          }}
        >
          📗 السداسي الثاني
        </button>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        <button
          onClick={() =>
            navigate(
              `/ar/annee/2/matiere/tajwid?semestre=${selectedSemester === 'semester1' ? 1 : 2}`
            )
          }
