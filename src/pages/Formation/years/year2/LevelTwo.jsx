// src/pages/Formation/years/year2/LevelTwo.jsx
...
function SectionSemester({ title, subjects, subjectKeys, semesterNumber }) {
  ...
  const handleClick = (subjectKey) => {
    if (introSubjects.includes(subjectKey)) {
      navigate(`/${lang}/intro/2/${subjectKey}?semestre=${semesterNumber}`);
    } else {
      navigate(
        `/${lang}/annee/2/matiere/${subjectKey}?semestre=${semesterNumber}`
      );
    }
  };
