...
function generateStats(data, subject, yearKey) {
  const result = { [yearKey]: { [subject]: {} } };
...
// === Parcours de tous les fichiers et années ===
for (const year of years) {
  const inputDir = path.resolve(`public/data/years/${year}/subjects_s1s2`);
  const outputDir = path.resolve(`src/datastat/years/${year}`);
  ...
  for (const subject of subjects) {
    ...
    const stats = generateStats(input, subject, year);

    fs.writeFileSync(outputPath, JSON.stringify(stats, null, 2), 'utf8');
    console.log(`✅ Stats générées pour ${subject} (${year}) : ${outputPath}`);
  }
}
